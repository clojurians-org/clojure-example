...
(ns hadoop.bolome.model.d_bolome_event
...
(defn -main []
  ...
  (as-> (<- [?dw-dt ?dw-ts ?dw-src-id
            ...
            ((hfs-delimited "hdfs://192.168.1.3:9000/user/hive/warehouse/ods.db/d_bolome_event" :delimiter "\001")
             :> ?dw-dt ?dw-ts ?dw-src-id
                ?event-id ?type-name ?event-name ?create-dt))
      $
      (?- (hfs-delimited "hdfs://192.168.1.3:9000/user/hive/warehouse/model.db/d_bolome_event"
